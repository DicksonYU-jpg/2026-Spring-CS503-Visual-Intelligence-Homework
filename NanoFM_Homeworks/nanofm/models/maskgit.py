...
class MaskGIT(nn.Module):
    ...
    def __init__(
        ...
        self.positional_embedding = nn.Parameter(torch.zeros(seq_len, dim)) # TODO: Define the learnable positional embedding
        self.mask_token = nn.Parameter(torch.zeros(dim)) # TODO: Define the learnable mask token
        
        self.trunk = TransformerTrunk(dim=dim,
                                      depth=depth,
                                      head_dim=head_dim,
                                      mlp_ratio=mlp_ratio,
                                      use_bias=use_bias) # TODO: Define the transformer trun
    ...
    def forward_model(self, x: torch.LongTensor, mask: torch.BoolTensor) -> torch.Tensor:
        ...
        input_embed[mask] = self.mask_token

        # TODO: Add the positional embeddings to the tokens
        pos_embed = self.positional_embedding[:L, :] # (L, D)
        tokens = input_embed + pos_embed
    ...
    def generate_random_mask(self, seq: torch.Tensor) -> torch.BoolTensor:
        ...
        # Decide the number of masked tokens for each sample in the batch, shape: [B, 1]
        num_mask = torch.randint(1, L + 1, (B, 1), device=device)

        # To create the mask, we can generate random scores for each token, 
        # sort them, and then select the top `num_mask` tokens to be masked. 
        # This way, we ensure that each sample has a different number of masked 
        # tokens without using a for loop.
        scores = torch.randn(B, L, device=device)
    ...
    def compute_ce_loss(self, logits: torch.Tensor, target_seq: torch.LongTensor, ignore_index: int = -100) -> torch.Tensor:
        ...
        target_seq = rearrange(target_seq, "B L -> (B L)")

        ce_loss = F.cross_entropy(input=logits, target=target_seq, ignore_index=ignore_index)
    ...
    def get_maskgit_schedule(self, mask: torch.BoolTensor, num_steps: int = 8) -> List[int]:
        ...
        step_size = total_tokens // num_steps
        schedule = [step_size] * num_steps
    ...
    def generate(
        ...
        for step, k in enumerate(schedule):
            ...
            confidence = torch.max(masked_logits, dim=-1)[0] # get the values
            ...
            top_k_indics = torch.topk(confidence, k)[1] # get the indices
            selected_positions = masked_indices[top_k_indics]
            ...
            samples, _ = sample_tokens(selected_logits, temp, top_p, top_k)
