...
class Attention(nn.Module):
    ...
    def forward(self, x: torch.Tensor, mask: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        qkv = rearrange(self.qkv_proj(x), 
                            "B L (qkv H hd) -> qkv B H L hd",
                            qkv = 3,
                            H = self.num_heads,
                            hd = self.head_dim)
        ...
        if mask is not None:
            ...
            attn = attn.masked_fill(mask == False, -float(torch.inf))
...
class Block(nn.Module):
    ...
    def __init__(self, dim: int, head_dim: int = 64, mlp_ratio: float = 4., use_bias: bool = False):
        ...
        self.mlp = Mlp(dim, mlp_hidden_dim, bias=use_bias) # TODO

    def forward(self, x: torch.Tensor, mask: Optional[torch.Tensor] = None) -> torch.Tensor:
        # Multi heads self attention and Add
...
class DecoderBlock(nn.Module):
    ...
    def forward(self, 
        ...
        x = x + self.cross_attn(q, context, xa_mask)
...
class TransformerTrunk(nn.Module):
    ...
    def __init__(
        ...
        self.blocks = nn.ModuleList([Block(dim, 
                                          ...
                                          mlp_ratio=mlp_ratio,
                                          use_bias=use_bias)
                                          for _ in range(depth)]) # TODO: Create a list of transformer blocks and wrap inside nn.ModuleList
...
class TransformerDecoderTrunk(nn.Module):
    ...
    def __init__(
        ...
        self.blocks = nn.ModuleList([DecoderBlock(dim,
                                                 head_dim=head_dim,
                                                 mlp_ratio=mlp_ratio,
                                                 use_bias=use_bias)
                                                 for _ in range(depth)]) # TODO: Create a list of transformer decoder blocks and wrap inside nn.ModuleList
